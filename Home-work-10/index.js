...
refs.followersBtn.addEventListener(
  "click",
  showUserData(followersList, followersBtn)
);
refs.repositoriesBtn.addEventListener(
  "click",
  showUserData(repositoriesList, repositoriesBtn)
);
...
function showUserData(dataTypeList, dataTypeBtn) {
  return () => {
    userSearch
      .fetchArticles(
        dataTypeList === followersList ? followersUrl : repositoriesUrl,
        ""
      )
      .then((data) => {
        if (data.message === "Not Found" || data.length === 0) {
          ...
          refs[dataTypeList].innerHTML = data.reduce((acc, current) => {
            return (
              acc +
              `<li class = "item"> <a class = "item-link" href=${
                current.html_url
              }>${
                dataTypeList === followersList ? current.login : current.name
              } </a></li>`
            );
          }, "");
        }
